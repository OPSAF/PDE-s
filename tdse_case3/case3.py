"""
Simulate 1D TDSE with constant force potential V(x)=F*x
Compares split-step spectral numerical solution with analytical accelerated Gaussian.

Produces: error metrics, expectation values, plots and animation.mp4
"""
import numpy as np
import matplotlib.pyplot as plt
from numpy.fft import fft, ifft, fftfreq
import imageio.v2 as imageio
from PIL import Image
import os
import matplotlib as mpl
mpl.rcParams['font.size'] = 12
plt.style.use('seaborn-v0_8-darkgrid')

# Parameters
F = 0.5
x0 = -2.0
k0 = 1.0
sigma = 1.0

# Spatial grid
Nx = 2048
L = 100.0
x = np.linspace(-L/2, L/2, Nx)
dx = x[1]-x[0]

# Temporal grid
T = 8.0
dt = 0.01
Nt = int(T/dt)
times = np.linspace(0, T, Nt+1)

def psi0(x):
    norm = (1/(2*np.pi*sigma**2))**0.25
    return norm * np.exp(- (x - x0)**2 /(4*sigma**2) + 1j*k0*x)

def psi_free(x, t):
    # free Gaussian analytical solution (units: m=1, hbar=1)
    w = sigma**2 + 1j * t / 2.0
    pref = (1/(2*np.pi*w))**0.25
    xc = x0 + k0 * t
    phase = 1j * (k0*(x - xc) - 0.5 * k0**2 * t)
    expg = np.exp( - (x - xc)**2 / (4.0 * w) + phase )
    return pref * expg

def psi_analytic_linear(x, t):
    # Analytical accelerated Gaussian: gauge transform of free solution
    shift = 0.5 * F * t**2
    phase_gauge = np.exp(-1j*(F * t * x - (F**2) * t**3 / 6.0))
    return phase_gauge * psi_free(x - shift, t)

def split_step(psi0x):
    # Split-step spectral method for V(x)=F*x
    psi = psi0x.copy()
    k = 2*np.pi*fftfreq(Nx, d=dx)
    Kfact = np.exp(-1j * (k**2) * dt / 2.0)
    V = F * x
    expV_half = np.exp(-1j * V * dt / 2.0)
    psi = expV_half * psi
    snapshots = [psi.copy()]
    for n in range(Nt):
        psi_k = fft(psi)
        psi_k *= Kfact
        psi = ifft(psi_k)
        psi *= expV_half
        if (n+1) % 10 == 0:
            snapshots.append(psi.copy())
    return snapshots

def compute_metrics(psi_num, psi_exact):
    # L2 error, max error, probability conservation, <x>, <p>
    dx = x[1]-x[0]
    diff = psi_num - psi_exact
    l2 = np.sqrt(np.sum(np.abs(diff)**2) * dx)
    maxe = np.max(np.abs(diff))
    prob = np.sum(np.abs(psi_num)**2) * dx
    # expectation x
    x_avg = np.sum(np.conj(psi_num) * x * psi_num).real * dx
    # momentum via spectral derivative
    psi_k = fft(psi_num)
    k = 2*np.pi*fftfreq(Nx, d=dx)
    ppsi = ifft(1j * k * psi_k)
    p_avg = np.sum(np.conj(psi_num) * (-1j * np.gradient(psi_num, dx))).real * dx
    return dict(l2=l2, maxe=maxe, prob=prob, x_avg=x_avg, p_avg=p_avg)

def main():
    psi_init = psi0(x)
    snapshots = split_step(psi_init)
    times_snap = np.linspace(0, T, len(snapshots))

    # prepare outputs
    os.makedirs('tdse_case3_outputs', exist_ok=True)
    frames = []
    metrics = []
    for i, psi_num in enumerate(snapshots):
        t = times_snap[i]
        psi_ex = psi_analytic_linear(x, t)
        m = compute_metrics(psi_num, psi_ex)
        metrics.append((t, m))
        if i % 1 == 0:
            dens_num = np.abs(psi_num)**2
            dens_ex = np.abs(psi_ex)**2
            # analytic center trajectory
            x_c = x0 + k0 * t + 0.5 * F * t**2
            fig, ax = plt.subplots(figsize=(8,4))
            ax.fill_between(x, 0, dens_ex, color='#1f77b4', alpha=0.25)
            ax.plot(x, dens_ex, color='#1f77b4', lw=2, label='analytic')
            ax.plot(x, dens_num, color='#ff7f0e', lw=1.5, label='numerical')
            ax.axvline(x_c, color='k', ls='--', lw=1)
            ax.set_xlim(x_c-12, x_c+12)
            ax.set_ylim(0, max(dens_ex.max(), dens_num.max())*1.3)
            ax.legend(loc='upper right')
            ax.set_xlabel('x')
            ax.set_ylabel('Probability density')
            ax.set_title(f't = {t:.3f}    L2={m["l2"]:.2e}  max={m["maxe"]:.2e}  prob={m["prob"]:.6f}')
            # annotate expectation values
            ax.text(0.02, 0.95, f"<x>={m['x_avg']:.3f}\n<p>={m['p_avg']:.3f}", transform=ax.transAxes,
                    verticalalignment='top', bbox=dict(boxstyle='round', fc='white', alpha=0.6))
            fname = f'tdse_case3_outputs/frame_{i:04d}.png'
            fig.savefig(fname, dpi=150)
            plt.close(fig)
            # load as PIL image for consistent sizing later
            pil_im = Image.open(fname).convert('RGBA')
            frames.append(pil_im)

    # normalize frame sizes and save GIF animation (prettier settings)
    gif_path = 'tdse_case3_outputs/animation.gif'
    if len(frames) == 0:
        print('No frames generated, skipping GIF')
    else:
        # determine max size
        widths, heights = zip(*(im.size for im in frames))
        max_w, max_h = max(widths), max(heights)
        norm_frames = []
        for im in frames:
            bg = Image.new('RGBA', (max_w, max_h), (255,255,255,255))
            # center
            x_off = (max_w - im.size[0]) // 2
            y_off = (max_h - im.size[1]) // 2
            bg.paste(im, (x_off, y_off), im)
            norm_frames.append(np.array(bg.convert('RGB')))
        imageio.mimsave(gif_path, norm_frames, duration=0.08)

    # save metrics summary
    with open('tdse_case3_outputs/metrics.txt', 'w') as f:
        for t, m in metrics:
            f.write(f"{t:.6f} {m['l2']:.6e} {m['maxe']:.6e} {m['prob']:.12f} {m['x_avg']:.6f} {m['p_avg']:.6f}\n")

    print('Outputs saved in tdse_case3_outputs')

if __name__ == '__main__':
    main()
